  ...
  constructor(serviceName) {
    ...
    const logger = winston.createLogger({
      ...
      format: winston.format.printf((info) => {
        ...
        let message = `${timeStamp()} | ${info.level} | From: ${this.service} | ${info.message}`;

        if (info.obj !== undefined) { //THIS LINE
            message = `${timeStamp()} | ${info.level} | From: ${this.service} | ${info.message} | ${JSON.stringify(info.obj)}`; //THIS LINE
            } //THIS LINE
        return message;
      }),
    });
